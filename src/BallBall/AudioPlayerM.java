...
public class AudioPlayerM {
  ...
  //-----建構子-----
     ...
     * @param file 傳入聲音檔案
     */
    public AudioPlayerM(final File file) {
	try {
     ...
     * @param url 傳入聲音URL
     */
    public AudioPlayerM(final URL url) {
	try {
	    init(url);
	} catch (final Exception ex) {
	    throw new RuntimeException(ex.getMessage());
     ...
     */
    public AudioPlayerM(final String str) {
	try {
  ...
  //-----初始化-----
     ...
     */
    void init(final URL url) throws Exception {
    //讀取音樂輸入串流
	try {
	...
	int bufferSize = (int) Math.min(audioInputStream.getFrameLength() * audioFormat.getFrameSize(), Integer.MAX_VALUE); //蝺抵�之撠���閮���之嚗隞亙�摮蝺抵�征������潭�府閬����捱摰�
